...
1-2. 1-1에서 얻은 부서번호로 IN연산를 통해 해당 부서에 속하는 직원정보 조회
SELECT*
FROM emp
WHERE deptno IN (20,30);
==> 서브 쿼리를 이용하면 하나의 SQL에서 실행가능
SELECT*
FROM emp
...
ANY, ALL
SMITH나 WARD 두사원의 급여중 아무 값이나 작은 급여를 받는 직원 조회
...
SMITH나 WARD 두사원의 급여보다 많은 급여를 받는 직원조회
SELECT*
FROM emp
...
NOT IN 연산자를 사용할 경우 서브 쿼리의 값에 NULL이 있는지 여부가 중요
아래 쿼리가 조회하는 결과는 어떤 의미인가? [mgr를 가지지 않는 empno를 조회]
NULL값을 가지는 행을 제거
SELECT*
FROM emp
...
매니져가 7698이면서 소속 부서가 30번인경우
매니져가 7839이면서 소속 부서가 10번인경우
(7698,30)
(7839,10)
mgr 컬럼과 deptno컬럼에 연관성이 없다 
(mge, deptno)
(7698,10)
(7698,30)
(7839,10)
...
emp 테이블만 사용할 경우 해당 직원의 소속 부서 이름을 알 수가 없다. ==> JOIN
...
상호연관 서브쿼리(corelated sub query)
    .메인 쿼리가 실행 되어야 서브 쿼리가 실행이 가능
    
비상호 연관 서브쿼리(non corelated sub query)
    .main 쿼리의 테이블을 먼저 조회 할 수도 있고
    sub 쿼리의 테이블을 먼저 조회 할 수도 있다. 
    ==> 오라클이 판단했을때 성능상 유리한 방향으로 실행 방향을 결정
모든 직원의 급여 평균보다 많은 급여를 받는 직원 정보
SELECT*
FROM emp
