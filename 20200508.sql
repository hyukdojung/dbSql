...
4. CHECK
    .NOT NULL
5. NOT NULL
    CHECK 제약 조건이지만 많이 사용하기 떄문에 별도의 KEYWORD를 제공
    

NOT NULL 제약조건
: 컬럼에 값이 NULL이 들어오는 것을 방지 하는 제약조건
...
dname 컬럼에 설정된 NOT NULL 제약조건에 의해 아래 INSERT 구문은 실패
...
현재 부서 테이블에서 99번 부서만 존재
foreign key 제약조건에 의해 emp, 테이블의 deptno 컬럼에는 99번 이외의 , 부서번호는 입력될수 없다. 
...
dept_test테이블의 99부서의 데이터를 지우면 어떻게 될까?
(9999,'brown',99) ===> 

부모 레코드(dept_test.deptno)를 참조하고 있는 자식 레코드(emp_test.deptno)가
존재하기 때문에 자식 레코드 입장에서는 데이터 무결성이 깨지게 되어 정상적으로 삭제 할 수 없다. 
DELETE dept_test
WHERE deptno=99;
...
2. 테이블 명세서가 정확하지 않으면 신규 개발자는 해당 내용을 모를 수가 있음
 ...
 신기하게도 emp테이블의 데이터가 삭제되거나 null로 설정될 경우 볼 수 있음
 
CHECK 제약조건
컬럼의 값을 제한하는 제약 조건

emp테이블에서 급여정보(sal) 를 관리할때 sal컬럼의 값음 0보다 작은 값이 들어가는 것은 로직적으로 이상함. 
...
이미 생성된 테이블을 수정 할 수도 있음
    1. 새로운 컬럼을 추가 
        ...
        **** 기존 생성되어 있는 컬럼 중간에는 새로운 컬럼을 추가 할 수가 없다. 
                ==> 기존 테이블을 삭제하고 컬럼순서를 조정한 새로운 테이블 생성 DDL로 
                    새롭게 테이블을 만들어야 함. 
                    운영중이라면 이미 데이터가 쌓여있을 가능성이 굉장히 높음
                    ==> 테이블을 생성할때 신중하게 컬럼 순서를 고려 
    2. 기존 컬럼 삭제, 컬럼 이름 변경, 컬럼 사이즈 변경(타입도 제한적으로 변경이 가능)
...
컬럼 타입 변경
위에서 추가한 hp 컬럼의 타입은 VARCHAR2(30)에서 DATE로 변경
...
컬럼 이름 변경
ALTER TABLE 테이블명 RENAME COLUMN 기존컬럼명 TO 신규 컬럼명;
